...
import { percentage, trimToDecimalPlaces } from '../utils'
...
const MAX_INVESTMENT = '2500000' * 10 ** USDT_DECIMALS
...
const MintGrid = () => {
    ...
    const max = (_input) => {
        ...
        if (_input === 'source') {
            ...
            if (selectedOptionTop?.label === 'ETH') {
                ...
                _sourceValue = trimToDecimalPlaces(
                    balances.USDTBalance / 10 ** USDT_DECIMALS,
                    18,
                )
                setInputValues((prev) => ({
                    ...prev,
    ...
    const half = (_input) => {
        ...
        if (_input === 'source') {
            ...
            if (selectedOptionTop?.label === 'ETH') {
                _sourceValue = trimToDecimalPlaces(
                    Web3.utils.fromWei(balances.ETHBalance) / 2,
                    18,
                )
                setInputValues((prev) => ({
                    ...prev,
                ...
                _sourceValue = trimToDecimalPlaces(
                    balances.USDTBalance / 10 ** USDT_DECIMALS / 2,
                    18,
                )
                setInputValues((prev) => ({
                    ...prev,
            ...
            let _destValue = trimToDecimalPlaces(
                Web3.utils.fromWei(balances.tokenBalance) / 2,
                18,
            )
